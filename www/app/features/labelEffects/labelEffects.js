...
angular.module('gapFront')
  .controller('LabelEffectsCtrl', function ($scope, IntegrationService, APIService, DrugService) {
    ...
    function updateList(resp){
      ...
      for(var i in res){
        if(contains($scope.effects, res[i].medical_term)) {
          $scope.effects.push(res[i].medical_term);
        }
      }
      for(var j=0; j<3; j++){
        addDisplayedEffect();
    ...
    function addDisplayedEffect(){
      var k = genRandomInt(0,$scope.effects.length);
      var add = $scope.effects.splice(k,1)[0];
      $scope.displayedEffects.push(add);
    }
